...
app.secret_key = os.getenv('SECRET_KEY', 'your-secret-key-here')  # In production, use a secure random key
...
def load_model_if_needed(model_type):
    ...
    # Resolve model paths from environment with sensible defaults
    inception_path = os.getenv('MODEL_INCEPTION_PATH', 'inception.h5')
    alexnet_path = os.getenv('MODEL_ALEXNET_PATH', 'Alexnet.h5')
    inception_url = os.getenv('MODEL_INCEPTION_URL')
    alexnet_url = os.getenv('MODEL_ALEXNET_URL')

    def _download_if_missing(dest_path: str, url: str):
        if not url:
            return
    ...
    if model_type == 'inception' and INCEPTION_MODEL is None:
        if not os.path.exists(inception_path):
            _download_if_missing(inception_path, inception_url)
        ...
        if not os.path.exists(alexnet_path):
            _download_if_missing(alexnet_path, alexnet_url)
